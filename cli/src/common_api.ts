...
import rmrkAbi from "./contracts/rmrk_contract.json";
...
export const getContract = async (address: string): Promise<ContractPromise> => {
...
export const getCatalogContract = async (address: string): Promise<ContractPromise> => {
    const api = await getApi();
...
export const doubleGasLimit = (api: ApiPromise | ApiBase<"promise">, weight: WeightV2): WeightV2 => {
...
export const executeCalls = async (
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        await new Promise((resolve) => {
            api.tx.utility.batchAll(batch).signAndSend(signer, (result: ISubmittableResult) => {
                if (result.isFinalized && !result.dispatchError) {
                    resolve(true);
                } else if (result.isFinalized && result.dispatchError) {
                    console.error(getErrorMessage(result.dispatchError));
